...
async def save_medication(medication_in: MedicationCreate, user_id: int = Depends(get_current_telegram_user_id), db: AsyncSession= Depends(get_db)):
    return await medication_service.create_medication(db, medication_in, user_id)

@router.get('', response_model = List[MedicationRead])
async def read_medication(user_id: int = Depends(get_current_telegram_user_id), db: AsyncSession = Depends(get_db)):
    medications = await medication_service.get_medication(db, user_id)
    return medications

@router.get('/{medication_id}', response_model = MedicationRead)
async def read_medication_by_id(medication_id: int, user_id: int = Depends(get_current_telegram_user_id), db: AsyncSession = Depends(get_db)):
...
async def update_medication(medication_id: int, medication_update: MedicationUpdate, user_id: int = Depends(get_current_telegram_user_id), db: AsyncSession = Depends(get_db)):
...
async def delete_medication(medication_id: int, user_id: int = Depends(get_current_telegram_user_id), db: AsyncSession = Depends(get_db)):
