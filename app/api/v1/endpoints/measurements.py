...
async def read_history(
    ...
    measurements = await measurement_service.get_history(db, user_id, limit=limit)
    return measurements or []
...
async def read_month_pressure(month: str, user_id: int = Depends(get_current_telegram_user_id), db: AsyncSession = Depends(get_db)):
...
async def read_date_pressure(date: str, user_id: int = Depends(get_current_telegram_user_id), db: AsyncSession = Depends(get_db)):
...
async def read_week_pressure(date: str, user_id: int = Depends(get_current_telegram_user_id), db: AsyncSession = Depends(get_db)):
    target_date = datetime.strptime(date, "%Y-%m-%d")
